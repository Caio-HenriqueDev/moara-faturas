...
# Importação condicional para compatibilidade com Vercel
try:
...
# Para Vercel, não criar pastas locais
if not os.getenv("VERCEL_ENV"):
    PASTA_DESTINO = "data/sample_emails"
    os.makedirs(PASTA_DESTINO, exist_ok=True)
else:
    PASTA_DESTINO = "/tmp/sample_emails"
    os.makedirs(PASTA_DESTINO, exist_ok=True)
...
def buscar_e_processar_emails():
    ...
    """
    # Para Vercel, retorna lista vazia se não houver credenciais
    if not EMAIL_USER or not EMAIL_PASS:
        print("Credenciais de email não configuradas para Vercel")
        return []
